...
interface ListaProps extends StackScreenProps<RootStackParamList,'Screen4'>{};

export const Screen4 = ({navigation}:ListaProps) => {
  ...
  const compareNumbers = () => {
    ...
    if (!isNaN(parsedNum1) && !isNaN(parsedNum2)) {
      if (parsedNum1 > parsedNum2) {
        setResult(`${parsedNum1} es mayor que ${parsedNum2}`);
      } else if (parsedNum2 > parsedNum1) {
        setResult(`${parsedNum2} es mayor que  ${parsedNum1}`)
  ...
  return (
    ...
    <View style={styles.container}>
      <Text style={styles.textWelcome}>MAYOR O IGUAL</Text>
